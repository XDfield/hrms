...
func DelSalaryRecordBySalaryRecordId(c *gin.Context, salaryRecordId string) error {
	...
	if err := db.Where("salary_record_id = ?", salaryRecordId).Delete(&model.SalaryRecord{}).
		Error; err != nil {
...
func GetSalaryRecordByStaffId(c *gin.Context, staffId string, start int, limit int) ([]*model.SalaryRecord, int64, error) {
	...
	db := resource.HrmsDB(c)
	if db == nil {
		log.Printf("GetSalaryRecordByStaffId: 数据库连接为空，鉴权失败")
		return nil, 0, resource.ErrUnauthorized // 返回鉴权失败错误
	}
	if start == -1 && limit == -1 {
		// 不加分页
		if staffId != "all" {
			err = db.Where("staff_id = ?", staffId).Order("salary_date desc").Find(&salaryRecords).Error
		} else {
			err = db.Order("salary_date desc").Find(&salaryRecords).Error
		}

	} else {
		// 加分页
		if staffId != "all" {
			err = db.Where("staff_id = ?", staffId).Offset(start).Limit(limit).Order("salary_date desc").Find(&salaryRecords).Error
		} else {
			err = db.Offset(start).Limit(limit).Order("salary_date desc").Find(&salaryRecords).Error
		}
	}
	if err != nil {
		return nil, 0, err
...
func GetSalaryRecordIsPayById(c *gin.Context, id int64) bool {
	...
	db := resource.HrmsDB(c)
	if db == nil {
		log.Printf("GetSalaryRecordIsPayById: 数据库连接为空，鉴权失败")
		return false // 鉴权失败时返回 false
	}
	db.Model(&model.SalaryRecord{}).Where("id = ? and is_pay = 2", id).Count(&total)
	return total != 0
}
...
func getStaffPhoneByStaffId(c *gin.Context, staffId string) int64 {
	...
	db := resource.HrmsDB(c)
	if db == nil {
		log.Printf("getStaffPhoneByStaffId: 数据库连接为空，鉴权失败")
		return 15521306934 // 鉴权失败时返回默认手机号
	}
...
func GetHadPaySalaryRecordByStaffId(c *gin.Context, staffId string, start int, limit int) ([]*model.SalaryRecord, int64, error) {
	...
	db := resource.HrmsDB(c)
	if db == nil {
		log.Printf("GetHadPaySalaryRecordByStaffId: 数据库连接为空，鉴权失败")
		return nil, 0, resource.ErrUnauthorized // 返回鉴权失败错误
	}
	if start == -1 && limit == -1 {
		// 不加分页
		if staffId != "all" {
			err = db.Where("staff_id = ? and is_pay = 2", staffId).Order("salary_date desc").Find(&salaryRecords).Error
		} else {
			err = db.Where("is_pay = 2").Order("salary_date desc").Find(&salaryRecords).Error
		}

	} else {
		// 加分页
		if staffId != "all" {
			err = db.Where("staff_id = ? and is_pay = 2", staffId).Offset(start).Limit(limit).Order("salary_date desc").Find(&salaryRecords).Error
		} else {
			err = db.Where("is_pay = 2").Offset(start).Limit(limit).Order("salary_date desc").Find(&salaryRecords).Error
		}
	}
	if err != nil {
		return nil, 0, err
	...
	var total int64
	db.Model(&model.SalaryRecord{}).Where("is_pay = 2").Count(&total)
	if staffId != "all" {
		total = int64(len(salaryRecords))
	}
	return salaryRecords, total, nil
