...
func PasswordQuery(c *gin.Context) {
	...
	db.Where("staff_id != 'root' and staff_id != 'admin'").Model(&model.Staff{}).Count(&total)
	psws = result
...
func buildPasswordQueryResult(c *gin.Context, staffId string, start int, limit int) ([]model.PasswordQueryVO, error) {
	...
	db := resource.HrmsDB(c)
	if db == nil {
		log.Printf("buildPasswordQueryResult: 数据库连接为空，鉴权失败")
	...
	if staffId == "all" {
		...
		if start == -1 && limit == -1 {
			// 不加分页
			err = db.Where("staff_id != 'root' and staff_id != 'admin'").Find(&loginList).Error
		} else {
			// 加分页
			err = db.Where("staff_id != 'root' and staff_id != 'admin'").Offset(start).Limit(limit).Find(&loginList).Error
		}
	} else {
		// 查询单个用户
		err = db.Where("staff_id != 'root' and staff_id != 'admin'").Where("staff_id = ?", staffId).First(&loginList).Error
	}
	if err != nil {
...
func convertStaffIdToName(c *gin.Context, staffId string) string {
	...
	db := resource.HrmsDB(c)
	if db == nil {
		log.Printf("convertStaffIdToName: 数据库连接为空，鉴权失败")
		return "" // 鉴权失败时返回空字符串
	}
	db.Raw("select staff_name from staff where staff_id = ?", staffId).Scan(&result)
